...
class RecEraser_MF(nn.Module):
    ...
    def compute_bpr_loss(self, users, pos_items, neg_items):
        ...
        Args:
            users:  embedding
            pos_items: positive  embedding
            neg_items: negative  embedding
    ...
    def batch_rating_local(self, users, items, local_num):
        ...
        if not isinstance(items, torch.Tensor):
            items = torch.from_numpy(np.array(items)).cuda().long()
        if not isinstance(users, torch.Tensor):
            users = torch.from_numpy(np.array(users)).cuda().long()
        u_e, i_e = self.emb_lookup(users, items, local_id=local_num)
        if self.bias_flag:
            return (torch.matmul(u_e, i_e.T) + self.local_biases[local_num]).detach().cpu().numpy()
        else:
            return torch.matmul(u_e, i_e.T).detach().cpu().numpy()
    ...
    def batch_rating_agg(self, users, items):
        ...
        u_e, _ = self.attention_based_agg(u_es, 0)
        i_e, _ = self.attention_based_agg(i_es, 1)
        u_e_drop = F.dropout(u_e, self.drop_prob, training=self.training)
        if self.bias_flag:
            return (torch.matmul(u_e_drop, i_e.T) + self.agg_bias).detach().cpu().numpy()
        else:
            return torch.matmul(u_e_drop, i_e.T).detach().cpu().numpy()

    def batch_rating(self, users, items):
        return self.batch_rating_agg(users, items)
    ...
    def compute_agg_model_bpr(self, users, pos_items, neg_items):
        with torch.no_grad():
        ...
        bpr_loss, _ = self.compute_bpr_loss(u_e_drop, pos_i_e, neg_i_e)
        reg_loss = self.decay_agg * (self.l2_loss(self.weights['trans_W']) + self.l2_loss(self.weights['trans_B']))
...
class RecEraser_LightGCN(nn.Module):
    ...
    def compute_bpr_loss(self, users, pos_items, neg_items, bias=None):
        ...
        if bias is None:
            ...
            pos_scores = torch.mul(users, pos_items).sum(dim=-1) + bias
            neg_scores = torch.mul(users, neg_items).sum(dim=-1) + bias
    ...
    def batch_rating_agg(self, users, items):  # all-ranking prediction
        ...
        u_es = torch.einsum('abc,bcd->abd', u_es, self.weights['trans_W']) + self.weights['trans_B']
        i_es = torch.einsum('abc,bcd->abd', i_es, self.weights['trans_W']) + self.weights['trans_B']
        # attention aggregation (same as agg_predict)
        u_e, _ = self.attention_based_agg(u_es, 0)
        i_e, _ = self.attention_based_agg(i_es, 1)
        if self.bias_flag:
            return (torch.matmul(u_e, i_e.T) + self.agg_bias).detach().cpu().numpy()
        else:
            return torch.matmul(u_e, i_e.T).detach().cpu().numpy()
    ...
    def compute_agg_model_bpr(self, users, pos_items, neg_items):
        """
        Aggregation training with BPR loss while freezing base embeddings.
        This mirrors compute_agg_model but uses BPR and handles negative items.
        Base user/item embeddings are detached; gradients flow to trans_W/trans_B and attention weights.
        """
        with torch.no_grad():
            u_es, pos_i_es = self.emb_lookup(users, pos_items)
            _, neg_i_es = self.emb_lookup(users, neg_items)
        ...
        reg_loss_agg = self.decay_agg * (self.l2_loss(self.weights['trans_W']) + self.l2_loss(self.weights['trans_B']))

        loss = bpr_loss + reg_loss + reg_loss_agg
        return loss, bpr_loss, (reg_loss + reg_loss_agg)
