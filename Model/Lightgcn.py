...
class LightGCN(nn.Module):
    ...
    def computer(self):
        ...
        for layer in range(self.n_layers):
            if self.A_split:
                ...
                #        
                all_emb = torch.stack(temp_emb, dim=0).mean(dim=0)
            else:
    ...
    def compute_bpr_loss(self, users, pos_items, neg_items):
        ...
        """
        # Embedding 
        (users_emb, pos_items_emb) = self.getEmbedding(users.long(), pos_items.long())
        (_, neg_items_emb) = self.getEmbedding(users.long(), neg_items.long())
