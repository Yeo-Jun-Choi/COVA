...
class Data_for_MF(object):
    def __init__(self, data_path, batch_size):
        ...
        self.train_original = pd.read_csv(data_path + "/train_original.csv", index_col=None)
        self.train_unlearn_original = pd.read_csv(data_path + "/train_unlearn_original.csv", index_col=None)
        self.valid = pd.read_csv(data_path + "/valid.csv", index_col=None)
        self.test = pd.read_csv(data_path + "/test.csv", index_col=None)
        self.data_size()

    def data_size(self):
        data = pd.concat([self.train_normal, self.train_unlearn_original, self.valid, self.test], axis=0, ignore_index=True)
    ...
    def set_train_mode(self, mode='full'):
        ...
        if mode == 'full':
            print("Train == train_normal + train_random")
            # self.train = pd.concat([self.train_normal, self.train_random], axis=0, ignore_index=True)
            self.train = self.train_original.copy()
        elif mode == 'original':
            print("Train == train_original")
            self.train = self.train_original.copy()
        else:
            print("Train == train_normal")
    ...
    def batch_generator(self):
        if self.train_loader is None:
            ...
            train_data = self.train[['user', 'item', 'label']].values.astype(np.float32)
...
class Data_for_RecEraser_MF(object):
    def __init__(self, data_path, batch_size, part_type=1, part_num=10, part_T=1, sisa=True):
        self.batch_size = batch_size
        self.path = data_path
        self.train_original = pd.read_csv(data_path + "/train_original.csv", index_col=None)
        self.train_normal = pd.read_csv(data_path + "/train_normal.csv", index_col=None)
        ...
        if self.sisa:
            try:
                ...
                train_all = pd.concat([self.train_normal, self.train_unlearn_original], axis=0, ignore_index=True)
                CC_, self.C_U, self.C_I = data_partition_3_withpath(self.path, train_all, part_num, part_T, itr_type='positive')
            ...
            try:
                with open(self.path + '/C_type-' + str(part_type) + '_num-' + str(part_num) + '_eraser.pk', 'rb') as f:
                    self.C, self.C_itr = pickle.load(f)
                with open(self.path + '/C_U_type-' + str(part_type) + '_num-' + str(part_num) + '_eraser.pk', 'rb') as f:
                    self.C_U = pickle.load(f)
                with open(self.path + '/C_I_type-' + str(part_type) + '_num-' + str(part_num) + '_eraser.pk', 'rb') as f:
                    self.C_I = pickle.load(f)
                    
            except Exception:  # partition
                train_all = pd.concat([self.train_normal, self.train_unlearn_original], axis=0, ignore_index=True)
                CC_, self.C_U, self.C_I = data_partition_1_withpath(self.path, train_all, part_num, part_T, itr_type='positive')
                if isinstance(CC_, tuple):
                    self.C, self.C_itr = CC_[0], CC_[1]

                with open(self.path + '/C_type-' + str(part_type) + '_num-' + str(part_num) + '_eraser.pk', 'wb') as f:
                    pickle.dump(CC_, f)
                with open(self.path + '/C_U_type-' + str(part_type) + '_num-' + str(part_num) + '_eraser.pk', 'wb') as f:
                    pickle.dump(self.C_U, f)
                with open(self.path + '/C_I_type-' + str(part_type) + '_num-' + str(part_num) + '_eraser.pk', 'wb') as f:
                    pickle.dump(self.C_I, f)
    ...
    def remove_unlearning_data(self):
        ...
        unlearn_data = self.train_unlearn_original[['user', 'item', 'label']].values.tolist()
...
class Data_for_RecEraser_LightGCN(object):
    def __init__(self, data_path, batch_size, part_type=1, part_num=10, part_T=1, sisa=True):
        ...
        if self.sisa:
            try:
                with open(self.path + '/C_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn.pk', 'rb') as f:
                    self.C, self.C_itr = pickle.load(f)
                with open(self.path + '/C_U_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn.pk', 'rb') as f:
                    self.C_U = pickle.load(f)
                with open(self.path + '/C_I_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn.pk', 'rb') as f:
                    self.C_I = pickle.load(f)
            except Exception:  # partition
                train_all = pd.concat([self.train_normal, self.train_unlearn_original], axis=0, ignore_index=True)
                ...
                with open(self.path + '/C_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn.pk', 'wb') as f:
                    pickle.dump(CC_, f)
                with open(self.path + '/C_U_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn.pk', 'wb') as f:
                    pickle.dump(self.C_U, f)
                with open(self.path + '/C_I_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn.pk', 'wb') as f:
                    pickle.dump(self.C_I, f)
        
        else:
            try:
                with open(self.path + '/C_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn_eraser.pk', 'rb') as f:
                    self.C, self.C_itr = pickle.load(f)
                with open(self.path + '/C_U_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn_eraser.pk', 'rb') as f:
                    self.C_U = pickle.load(f)
                with open(self.path + '/C_I_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn_eraser.pk', 'rb') as f:
                    self.C_I = pickle.load(f)
            except Exception:  # partition
                train_all = pd.concat([self.train_normal, self.train_unlearn_original], axis=0, ignore_index=True)
                ...
                with open(self.path + '/C_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn_eraser.pk', 'wb') as f:
                    pickle.dump(CC_, f)
                with open(self.path + '/C_U_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn_eraser.pk', 'wb') as f:
                    pickle.dump(self.C_U, f)
                with open(self.path + '/C_I_type-' + str(part_type) + '_num-' + str(part_num) + '_lightgcn_eraser.pk', 'wb') as f:
                    pickle.dump(self.C_I, f)
    ...
    def set_train_mode(self, mode='full', sisa=True):
        self.train_mode = mode
        if mode == 'retraining':  # 设置train，移去unlearn，更新Graph
            self.train = self.train_normal
            self.remove_unlearning_data()
            for i in range(self.part_num):
                self.Graph[i] = self.getSparseGraph_mode(i, mode, sisa)
    ...
    def getSparseGraph_mode(self, local_id, mode, sisa=True):
...
class Data_for_LightGCN(object):
    def __init__(self, config, path):
        ...
        train_original_file = path + "/train_original.csv"
        self.train_unlearn_original = pd.read_csv(path + "/train_unlearn_original.csv", index_col=None)
        ...
        self.train_original = pd.read_csv(train_original_file)[['user', 'item', 'label']]
        # self.train = pd.read_csv(train)[['user', 'item', 'label']]
        self.valid = pd.read_csv(valid_file)[['user', 'item', 'label']]
        self.test = pd.read_csv(test_file)[['user', 'item', 'label']]
        
        # backward compatibility: expose unlearning set with the expected name
        self.train_unlearn_original = pd.read_csv(path + "/train_unlearn_original.csv", index_col=None)

        self.n_users = 1 + max([self.train_normal['user'].max(), self.train_unlearn_original['user'].max(), self.valid['user'].max(), self.test['user'].max()])
        self.n_items = 1 + max([self.train_normal['item'].max(), self.train_unlearn_original['item'].max(), self.valid['item'].max(), self.test['item'].max()])

        self.testDataSize = self.test.shape[0]
        self.validDataSize = self.valid.shape[0]
        self.train_normal_size = self.train_normal.shape[0]
        self.train_random_size = self.train_unlearn_original.shape[0]
    ...
    def getSparseGraph_mode_b(self, mode, ratio_flag=0.01, recomputed=False):
        ...
        if self.ChangedGraph is None:
            try:
                ...
                train_unlearn_pos = self.train_unlearn_original[self.train_unlearn_original['label']>0].values[:, 0:3]
                ...
                if unlearned_idx.size > 0:
                    print("changed ratio info: (mean, median, max, min):(%.5f, %.5f, %.5f, %.5f)" % (
                        changed_ratio[unlearned_idx].mean(),
                        np.median(changed_ratio[unlearned_idx]),
                        changed_ratio[unlearned_idx].max(),
                        changed_ratio[unlearned_idx].min()
                    ))
                else:
                    print("no positive interactions in train_random; skip changed ratio stats")
                ...
                if changed_u.size == 0 and changed_i.size == 0:
                    #   downstream   DataFrame 
                    self.changed_data = pd.DataFrame(columns=['user','item','label'])
                else:
