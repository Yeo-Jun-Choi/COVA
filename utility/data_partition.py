...
def data_partition_1_withpath(data_path, train, k, T, itr_type='all'):
    ...
    if itr_type == 'all':
        ...
        data = train.loc[train['label'].isin([1]), ['user','item','label']].values.astype(int)
    else:
        raise NotImplementedError("you must set the ity_type as all or positive!")
    
    # Randomly select k centroids
    data =  data.tolist()
    target_per_cluster = len(data)
    ...
    for tt in range(T):
        ...
        for i in range(len(Scores)):
            if Scores[i][0][0] not in fl:
                ...
                if C_num[Scores[i][0][1]] < target_per_cluster:
        ...
        for i in range(k):
            ...
            centroembs_next.append([np.mean(temp_u, axis=0), np.mean(temp_i, axis=0)])
