...
def main(config_args):
    ...
    config = dict()
    data_generator = Data_for_RecEraser_LightGCN(args.data_path + args.dataset + '/' + args.attack, args.batch_size, args.part_type, args.part_num, args.part_T, False)
    data_generator.set_train_mode(args.data_type, sisa=False)
    ...
    if args.save_flag == 1:
        weights_save_path = './Weights/LightGCN_Eraser_BPR/lightgcn_'
    ...
    original_save_name = f'./Weights/LightGCN/LightGCN_lr-{0.001}-embed_size-{args.embed_size}-batch_size-2048-data_type-original-dataset-{args.dataset}-attack-{args.attack}-seed-{args.seed}-init_std-0.001-gcn_layers-{getattr(args, "gcn_layers", 1)}-n_neg-1-m.pth'
