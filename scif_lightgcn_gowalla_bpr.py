...
class influence_unlearn(nn.Module):
    def __init__(self, save_name, if_epoch=100, if_lr=1e-2, k_hop=0, init_range=1e-4) -> None:
    ...
    def compute_hessian_with_test(self, model=None, data_generator=None, original_model=None, args=None, baseline_valid_recall_20=None):
        # ensure save dir exists
        save_dir = os.path.dirname(self.save_name)
        if len(save_dir) > 0:
        ...
        def unlearn_loss_fun(para1, para2):
            ...
            adj_graph = model.Graph
            user_emb_gnn, item_emb_gnn = model.F_computer(u_para, i_para, adj_graph)
            user_embs = user_emb_gnn[unlearned_data[:,0].long()]
            item_embs = item_emb_gnn[unlearned_data[:,1].long()]
            scores = torch.mul(user_embs, item_embs).sum(dim=-1)
        ...
        label_path = os.path.join(args.data_path, args.dataset, args.attack, 'avg_labels.npy') if args is not None else os.path.join('Data','Process','Gowalla','0.01','avg_labels.npy')
        avg_labels = np.load(label_path)
        if avg_labels.ndim != 1:
            avg_labels = avg_labels.reshape(-1)
        ...
        def unlearn_loss_fun_avg(para1, para2):
            ...
            item_embs = item_emb_gnn[unlearned_data[:,1].long()]
            scores = torch.mul(user_embs, item_embs).sum(dim=-1)
            if scores.shape[0] != avg_labels.numel():
                raise ValueError(f"avg_labels length {avg_labels.numel()} != scores length {scores.shape[0]}. label_path={label_path}")
        ...
        unlearn_grad = unlearn_grad - unlearn_grad_avg + total_grad * len(data_generator.train_original[['user','item','label']].values)
        ...
        for if_ep in range(self.if_epoch):
            ...
            with torch.no_grad():
                ...
                if original_model is not None and current_drop_ratio > best_metric:
...
def main(config_args=None):
    ...
    save_name = "Weights/LightGCN_SCIF/lightgcn_dataset_{}_attack_{}_lr_{}_khop_{}_emb_{}_gcnlayers_{}.pth".format(
    ...
    unlearn.compute_hessian_with_test(model=model, data_generator=data_generator, original_model=original_model, args=args, baseline_valid_recall_20=valid_metrics['recall_at_20'])
